...
class Horde_Form_Type_email extends Horde_Form_Type {
    ...
    function isValid(&$var, &$vars, $value, &$message)
    {
        if ($var->isRequired() && empty($value)) {
            ...
            $parsed_email = $parser->parseAddressList($value, false, true);
    ...
    function about()
    {
        ...
        $about['params'] = array(
            'allow_multi' => array('label' => _("Allow multiple addresses"),
                                   'type'  => 'boolean'),
            'strip_domain' => array('label' => _("Strip domain from the address"),
                                    'type' => 'boolean'),
...
class Horde_Form_Type_hourMinuteSecond extends Horde_Form_Type {
    ...
    function getTimeOb($time_in)
    {
        ...
        if (is_array($time_in)) {
            if (!$this->emptyTimeArray($time_in)) {
                $time_in = sprintf('1970-01-01 %02d:%02d:%02d', $time_in['hour'], $time_in['minute'], $time_in['second']);
            }
        }
    ...
    function about()
    {
        return array('name' => _("Time selection"));
    }
...
class Horde_Form_Type_selectFiles extends Horde_Form_Type {
    ...
    function init($link_text = null, $link_style = '', $icon = false,
                  $selectid = '')
    {
...
class Horde_Form_Type_assign extends Horde_Form_Type {
    ...
    function getOptions($side, $formname)
    {
        ...
        foreach ($values as $key => $val) {
            ...
            if ($headers) {
                ...
                $html .= ' ondblclick="Horde_Form_Assign.move(\'' . $formname .
             '\', ' . (int)$side . ');"';
            }
            $html .= '>' . $val . '</option>';
        }
    ...
    {
        $about = array();
        $about['name'] = _("Assignment columns");
        $about['params'] = array(
...
class Horde_Form_Type_obrowser extends Horde_Form_Type {
    ...
    {
        $about = array();
        $about['name'] = _("Relationship Browser");
        return $about;
    }
