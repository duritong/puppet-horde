...
class IMP_Maillog {
    ...
    function log($type, $msg_ids, $data = null)
    {
        ...
        foreach ($msg_ids as $val) {
            ...
            switch ($type) {
            case 'forward':
                $r = $history->log($id, array('recipients' => $data, 'action' => 'forward'));
                break;

            case 'mdn':
                $r = $history->log($id, array('type' => $data, 'action' => 'mdn'));
                break;

            case 'redirect':
                $r = $history->log($id, array('recipients' => $data, 'action' => 'redirect'));
                break;

            case 'reply':
                $r = $history->log($id, array('action' => 'reply'));
                break;
            }
