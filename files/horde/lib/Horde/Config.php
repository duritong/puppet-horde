...
class Horde_Config {
    ...
    function readXMLConfig($custom_conf = null)
    {
        if (is_null($this->_xmlConfigTree) || $custom_conf) {
            ...
            /* Load the DOM object. */
            $doc = Horde_Config_Document::factory($path . '/conf.xml');
        ...
        foreach ($children as $node) {
            ...
            case 'configmultienum':
                ...
                $conf[$name] = array('_type' => 'multienum',
                                     ...
                                     'values' => $values,
                                     'desc' => $desc,
                                     'default' => Horde_Array::valuesToKeys($this->_default($curctx, explode(',', $this->_getNodeOnlyText($node)))));
                break;
    ...
    function _configSQL($ctx, $node = null, $switchname = 'driverconfig')
    {
        ...
        $charset = array(
            ...
            'default' => $this->_default($ctx . '|charset', 'iso-8859-1'));
        ...
        $config = array(
            ...
            'default' => $this->_default($ctx . '|' . (isset($node) ? $node->get_attribute('switchname') : $switchname), 'horde'),
            'switch' => array(
                'horde' => array(
                    'desc' => 'Horde defaults',
                    'fields' => array()),
                'custom' => array(
                    ...
                    'fields' => array(
                        md5(microtime()) => array(
                            '_type' => 'description',
                            'desc' => 'Any parameters that the storage driver needs.'),
    ...
    function _configVFS($ctx, $node)
    {
        ...
        $config =
            array(
                ...
                'default' => $this->_default($ctx . '|' . $node->get_attribute('switchname'), 'horde'),
                'switch' => array(
                    'none' => array(
                        'desc' => 'None',
                        'fields' => array()),
...
class Horde_Config_Document extends Horde_Config_Node {
    ...
    function factory($file)
    {
        if (function_exists('domxml_open_file')) {
            ...
            return $doc = new Horde_Config_Document($file);
        } else {
            Horde::fatal('DOM support not present.', __FILE__, __LINE__);
        }
    }

    function Horde_Config_Document($filename)
    {
        $this->node = new DOMDocument();
        $this->node->load($filename);
    }

    function root()
    {
        return $root = new Horde_Config_Element($this->node->documentElement);
    }

}

class Horde_Config_Node {

    var $node;

    function Horde_Config_Node($aDomNode) {$this->node=$aDomNode;}
    ...
    function child_nodes()
    {
        ...
        if (isset($nodeList)) {
            ...
            while ($node = $nodeList->item($i)) {
                $nodeSet[] = new Horde_Config_Element($node);
                $i++;
            }
        }
        return $nodeSet;
    ...
    {
        return new Horde_Config_Element($this->node->firstChild);
    }
    ...
    {
        return new Horde_Config_Element($this->node->nextSibling);
    }
...
class Horde_Config_Element extends Horde_Config_Node {
