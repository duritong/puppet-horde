<?php
/**
 * $Horde: horde/services/portal/sidebar.php,v 1.4.2.16 2007/03/09 17:18:49 jan Exp $
 *
 * Copyright 1999-2007 Charles J. Hagenbuch <chuck@horde.org>
 * Copyright 1999-2007 Jon Parise <jon@horde.org>
 * Copyright 2003-2007 Michael Pawlowsky <mjpawlowsky@yahoo.com>
 *
    ...
    if (Auth::isAdmin()) {
        ...
        foreach ($registry->listApps() as $app) {
            ...
            foreach ($list as $method => $vals) {
                ...
                $icon = isset($vals['icon']) ? $registry->getImageDir($app) . '/' . $vals['icon'] : $registry->get('icon', $app);

                $menu['administration_' . $app . '_' . $method] = array(
                    'name' => $name,
                    'icon' => $icon,
                    'status' => 'active',
                    'menu_parent' => 'administration',
                    'url' => Horde::url($registry->applicationWebPath($vals['link'], $app)),
                    );
            }
        }
    }
