 ...
 * remote mail servers.
 *
 * $Horde: imp/lib/Fetchmail.php,v 1.41.8.12 2007/01/02 13:54:56 jan Exp $
 *
...
class IMP_Fetchmail {
    ...
    function _processMailMessage($header, $body)
    {
        ...
        if ($GLOBALS['conf']['hooks']['fetchmail_filter']) {
            ...
            if (function_exists('_imp_hook_fetchmail_filter')) {
                $msg = call_user_func('_imp_hook_fetchmail_filter', $msg);
            }
        }
    ...
    {
        return @imap_append($GLOBALS['imp']['stream'], IMP::serverString($this->_params['lmailbox']), $msg);
    }
