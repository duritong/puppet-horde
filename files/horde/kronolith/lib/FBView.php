 ...
 *
 * $Horde: kronolith/lib/FBView.php,v 1.10.10.11 2007/01/02 13:55:05 jan Exp $
 *
...
class Kronolith_FreeBusy_View {
    ...
    function render($day = null)
    {
        ...
        if ($prefs->getValue('show_fb_legend')) {
            ...
            $template->set('legend', $template->fetch(KRONOLITH_TEMPLATES . '/fbview/legend.tpl'));
        } else {
            $template->set('legend', '');
        }

        return $html . $template->fetch(KRONOLITH_TEMPLATES . '/fbview/section.tpl') . '</div>';
    }
