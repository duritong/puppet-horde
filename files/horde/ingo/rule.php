<?php
/**
 * $Horde: ingo/rule.php,v 1.63.8.8 2007/01/02 13:55:02 jan Exp $
 *
...
foreach ($rule['conditions'] as $cond_num => $condition) {
    $lastfield = ($lastcond == $cond_num) ? true : false;
    ...
    if ($conf['rules']['userheader']) {
        $field_select .= "<option value=\"\"> --------- </option>\n";
        $field_select .= '<option value="' . INGO_USER_HEADER . '"' . ((!$option_selected) ? ' selected="selected"' : '') . '>' . _("Self-Defined Header") . (($lastfield) ? '' : ':') . "</option>\n";
        if (!$option_selected) {
            $header_entry = true;
            $userheader = Util::getFormData('userheader');
            if (empty($userheader)) {
    ...
    $match_value = '<input name="value[' . (int)$cond_num . ']" size="40" value="' . $value . '" />';

    switch ($testOb->type) {
    case 'text':
        if ($ingo_script->caseSensitive()) {
            $match_value .= '<input type="checkbox" name="case[' . (int)$cond_num . ']" value="1" ';
            $match_value .= (isset($condition['case']) && $condition['case']) ? 'checked="checked" ' : '';
            $match_value .= '/> ' . _("Case Sensitive");
        }
        break;
    }
    require INGO_TEMPLATES . '/rule/filter.inc';
...
$actionselect = '';
$actionlist = $ingo_script->availableActions();
...
case 'text':
case 'int':
    $actionvalue = '<input name="actionvalue" size="40" value="' . htmlspecialchars($rule['action-value']) . '" />';
    break;
}
