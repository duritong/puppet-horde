...
define('MIMP_IMAPCLIENT_TAGGED', 0);
define('MIMP_IMAPCLIENT_UNTAGGED', 1);
define('MIMP_IMAPCLIENT_CONTINUATION', 2);

/**
 * The MIMP_IMAPClient:: class enables connection to an IMAP server through
 * built-in PHP functions.
 *
 * TODO: This should eventually be moved to Horde 4.0/framework.
 *
 * $Horde: mimp/lib/IMAP/Client.php,v 1.19.2.1 2007/01/02 13:55:09 jan Exp $
 *
...
class MIMP_IMAPClient {
     ...
     */
    function MIMP_IMAPClient($host, $port, $protocol)
    {
        $this->_host = $host;
        $this->_port = $port;
     ...
     * 'type' - Either MIMP_IMAPCLIENT_TAGGED, MIMP_IMAPCLIENT_UNTAGGED, or
     *          MIMP_IMAPCLIENT_CONTINUATION
     * </pre>
     */
    function _runCommand($query)
    {
        ...
        if ($ob->type != MIMP_IMAPCLIENT_CONTINUATION) {
            $this->_currtag = null;
        }
    ...
    function _parseLine()
    {
        ...
        case '+':
            ...
            $ob->response = 'OK';
            $ob->type = MIMP_IMAPCLIENT_CONTINUATION;
            break;
        ...
        case '*':
            ...
            $ob->type = MIMP_IMAPCLIENT_UNTAGGED;
            $ob2 = $this->_parseLine();
            if ($ob2->response != 'OK') {
                $ob = $ob2;
            } elseif ($ob2->type == MIMP_IMAPCLIENT_UNTAGGED) {
                $ob->message .= "\n" . $ob2->message;
            }
            break;
        ...
        default:
            $tmp = explode(' ', $read[1], 2);
            $ob->type = MIMP_IMAPCLIENT_TAGGED;
    ...
    function _login($username, $password, $method)
    {
        ...
        case 'login':
            ...
            if (!is_a($read, 'PEAR_Error') &&
                ($read->type == MIMP_IMAPCLIENT_CONTINUATION)) {
     ...
     *     [name] => (string)
     *     [delimiter] => (string)
     *     [type] => [personal|others|shared] (string)
     *     [hidden] => (boolean)
     *   )
     *
     *   [foo2] => Array
    ...
    function namespace($additional = array())
    {
        ...
        if ($this->queryCapability('NAMESPACE')) {
            ...
            if (($read->type == MIMP_IMAPCLIENT_UNTAGGED) &&
