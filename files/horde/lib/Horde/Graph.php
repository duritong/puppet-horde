...
class Horde_Graph {
    ...
    function display()
    {
        ...
        $cache->set($cid, serialize(array('data' => $this->draw(false),
                                          'ctype' => $this->img->getContentType())));

        $url = Horde::url($registry->get('webroot', 'horde') . '/services/cacheview.php');
     ...
     */
    function initialize($minY = false, $stepLabelsY = false)
    {
        // Load a Horde_Image canvas to draw on.
        ...
        $this->_factorY = $this->_graphHeight / $this->_diffY;
        ...
        if ($stepLabelsY === false) {
            ...
            $this->_deltaTicksY = $stepLabelsY;
        }

        return true;
    }

    function draw($display = true, $minY = false, $deltaLabelsY = false)
    {
        $result = $this->initialize($minY, $deltaLabelsY);
    ...
    {
        for ($i = 0; $i <= $this->_gridY; $i++) {
            $y = $this->translateY($this->_minY + $this->_deltaTicksY * $i);
    ...
    function drawLabelsY()
    {
        ...
        foreach ($this->_labelsY as $i => $label) {
            $y = $this->translateY($this->_minY + $i * $this->_deltaTicksY);
    ...
    {
        if ($y) {
            return $this->_graphBottom - (($y - $this->_minY) * $this->_factorY);
        } else {
     ...
     *                             number in the dataset is used if it's
     *                             higher.
